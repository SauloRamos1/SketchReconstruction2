...
public:    
    ...
    ///
    /// Verifies whether two curves are the same.  Returns true if 
    /// all vertices are equal and both curves have the same topology.
    /// \param rhs: curve to compare.
    virtual bool operator==( const PolygonalCurve &rhs ) {
        bool equal = true;

        if ( this->pPoints.size() != rhs.pPoints.size() )
        {
            return false;
        }

        equal &= ( this->isClosed() == rhs.isClosed() );

        if ( equal )
        for ( size_t i = 0; i < this->pPoints.size(); ++i )
        {
            equal &= ( this->pPoints[i] == rhs.pPoints[i] );
        }
    ...
    ///
    /// Verifies whether two curves are not the same.  Returns true if 
    /// either any vertex differ or if the topologies differ.
    /// \param rhs: curve to compare.
    virtual bool operator!=( const PolygonalCurve &rhs )
    {
        return !operator==(rhs);
    ...
    virtual void lineFilter( real step = 0.1 , unsigned int numberOfInteractions = 5 ){
        if (size() == 0) return;
